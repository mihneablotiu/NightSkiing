...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix *= glm::rotate(modelMatrix, (float) glm::radians(30.0), glm::vec3(1, 0, 0));
        RenderMesh(meshes["box2"], shaders["VertexNormal"], modelMatrix);
    }

    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 0.2f, 0));
        modelMatrix *= glm::scale(modelMatrix, glm::vec3(1.2, 1.2, 1.2));
        modelMatrix *= glm::rotate(modelMatrix, (float)glm::radians(45.0), glm::vec3(1, 0, 0));
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_T)
    {
        ...
        glm::ivec2 resolution = window->GetResolution();
        projectionMatrix = glm::ortho((double) left, (double) right, -5.0, 5.0, 0.5, 100.0);
    }
