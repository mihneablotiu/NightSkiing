...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int lightPosition = glGetUniformLocation(shader->program, "light_position");
    glUniform3f(lightPosition, this->lightPosition.x, this->lightPosition.y, this->lightPosition.z);

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    int eyePositionInt = glGetUniformLocation(shader->program, "eye_position");
    glUniform3f(eyePositionInt, eyePosition.x, eyePosition.y, eyePosition.z);

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    int shiningness = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(shiningness, materialShininess);
    ...
    int colorInt = glGetUniformLocation(shader->program, "object_color");
    glUniform3f(colorInt, color.x, color.y, color.z);
